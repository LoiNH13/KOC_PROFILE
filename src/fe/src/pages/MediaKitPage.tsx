...
import { KOC_DATA, STATS, NICHES, AUDIENCE, PACKAGES, BRANDS } from '@/data/koc-data'
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, letterSpacing: -0.5, lineHeight: 1 }}>Linh Chi</div>
              <div style={{ fontSize: 12, color: 'var(--color-clay-ink-soft)', fontWeight: 500 }}>
                {KOC_DATA.handle} · {KOC_DATA.location}
              </div>
            </div>
          </div>
          <div style={{
        ...
        <div style={{ padding: '0 36px', display: 'grid', gridTemplateColumns: '1fr 180px', gap: 20, alignItems: 'center' }}>
          <div>
            <div style={{
              fontFamily: 'var(--font-display)', fontSize: 40, fontWeight: 600, letterSpacing: -1.5,
              lineHeight: 1, color: 'var(--color-clay-ink)', marginBottom: 10,
            }}>
              {lang === 'vi' ? (
                <>KOC mới · <em style={{ color: 'var(--color-clay-accent-ink)', fontStyle: 'italic' }}>Giọng thật</em></>
              ) : (
                <>Emerging KOC · <em style={{ color: 'var(--color-clay-accent-ink)', fontStyle: 'italic' }}>Real voice</em></>
              )}
            </div>
          ...
          <div style={{ padding: 16, borderRadius: 18, background: 'var(--color-clay-surface)', boxShadow: 'var(--shadow-clay)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
              {AUDIENCE.platforms.map((p, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '4px 0', borderBottom: '1px solid var(--color-clay-border)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                {AUDIENCE.cities.map((c, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, padding: '3px 0' }}>
                    <span style={{ color: 'var(--color-clay-ink-soft)' }}>{typeof c.name === 'object' ? c.name[lang] : c.name}</span>
